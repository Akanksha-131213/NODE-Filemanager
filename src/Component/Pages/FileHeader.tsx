...
import { update_File } from "../../redux/Graphql/query.js";
const FileHeader = ({ fileName, fileId, fileData, prevFileData }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [updateFile, { error }] = useMutation(update_File);
  return (
      ...
      <ul className="navbar-nav ms-auto me-5">
        <li className="nav-item mx-2">
          <button
            ...
            onClick={() => {
              updateFile({
                variables: { id: fileId, data: fileData },
              });
              dispatch(updateFileData(fileId, fileData));
            }}
          >
            <img
              src="https://img.icons8.com/external-flaticons-lineal-color-flat-icons/64/null/external-save-web-flaticons-lineal-color-flat-icons-4.png"
              alt=""
              height={40}
              width={40}
            />
